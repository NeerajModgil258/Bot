...
client.on("message", async message => {
  ...
  .setDescription('**`Blacklist expires:`** ||' + `${blTime1}` + '||' + `\n` + '**`Blacklist reason:`** ' + `${blReason}` + `\n\n` + `*If you think this is a mistake [Click Here](https://tinyurl.com/alfredblacklistappeal) to appeal against the blacklist.*`)
  .setColor('#FFFFFF')
  ...
  .setDescription('**`Blacklist expires:`** ||' + `${blGuildTime1}` + '||' + `\n` + '**`Blacklist reason:`** ' + `${blGuildReason}` + `\n\n` + `*If you think this is a mistake [Click Here](https://tinyurl.com/alfredserverblacklistappeal) to appeal against the blacklist. Appealing by multiple users won't increase the chances of being whitelisted.*`)
  .setColor('#FFFFFF')
...
if(bl === true) return message.channel.send(blEmbed)
 let bls = await db.fetch(`blguild_${message.guild.id}`)
if(bls === true) return message.channel.send(blGuildEmbed) 
